...
fn decode_create_blog_req(
  ...
  decode.run(json, {
    ...
    let slug = utils.slugify(raw_slug)
...
pub fn create_blog(json: dynamic.Dynamic, ctx: auth.Context) -> Response {
  verify_user(ctx)
  |> result.then(decode_create_blog_req(_, json))
...
pub fn update_blog(
  ...
  verify_user(ctx)
  |> result.then(decode_update_blog_req(_, blog_id, json))
...
fn encode(
  ...
  let stamp = utils.from_pog_timestamp(created_at) |> birl.to_iso8601
  let update_stamp = utils.from_pog_timestamp(updated_at) |> birl.to_iso8601
  json.object([
